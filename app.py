...
# --- Load rules ---------------------------------------------------------------
DEFAULT_RULES_PATHS = [
    pathlib.Path("/mnt/data/eligibility_package/eligibility_rules.json"),  # if running here
    pathlib.Path("eligibility_rules.json"),                                 # if copied next to app
]
...
def decide(flags):
    ...
    # Provisional (pre run-in)
    if flags["inclusion_all"] and not flags["exclusion_any"]:
        # If run-in metrics provided, finalize
        if flags["runin_ok"]:
